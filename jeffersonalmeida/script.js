// --- CONFIGURAÇÃO ---
// Substitua pela URL do seu Google Apps Script (Executável)
const API_URL = "https://script.google.com/macros/s/AKfycbwdK1wGnKpC0YV5XXg9tZmNs-cX5eYAwUG3FNAbUfyhHN_YCmQbzyFL3NWcXSzZSvak/exec"; 
...
const services = [
    { 
        ...
        duration: "45min", 
        icon: "scissors",
        desc: "Tesoura ou máquina. Lavagem e finalização."
    },
    { 
        id: 2, 
        name: "Barba Completa", 
        ...
        duration: "30min", 
        icon: "user",
        desc: "Ritual completo com massagem facial."
    },
    { 
        id: 3, 
    ...
    { 
        ...
        duration: "15min", 
        icon: "zap",
        desc: "Apenas alinhamento dos contornos."
    }
];
...
function init() {
    ...
    // Deixa o container limpo inicialmente
...
// 1. Busca Dias Ocupados (Sem travar UI)
async function fetchBusyDays(month, year) {
    ...
    try {
        ...
        console.error("Erro dias ocupados:", error);
    }
}

// 2. Busca Horários (Com Controle de UI do Calendário)
async function fetchTimeSlots(dateObj) {
    const container = document.getElementById('time-slots-container');
    // Seleciona o botão do calendário dentro do passo 3 para esconder/mostrar
    const calendarBtn = document.querySelector('#step-3 button'); 
    
    // --- ESTADO DE CARREGAMENTO ---
    // 1. Esconde o botão do calendário
    if(calendarBtn) calendarBtn.style.display = 'none';
    
    // 2. Mostra loading no container
    container.innerHTML = `
    ...
    // Fallback Mock (Se não tiver API)
    if (API_URL.includes("SUA_URL")) {
        ...
        }, 1500); // Delay artificial para ver o loading
        return;
    }
    ...
    try {
        ...
        // --- ESTADO DE SUCESSO ---
        // 1. Mostra o botão do calendário de volta
        if(calendarBtn) {
            calendarBtn.style.display = 'flex';
            calendarBtn.classList.add('animate-fadeIn'); // Efeito suave
        }

        // 2. Renderiza horários
        if (data.slots && data.slots.length > 0) {
            ...
            container.innerHTML = `
                <div class="flex flex-col items-center justify-center py-8 text-gray-500 animate-fadeIn">
                    ...
                    <span class="text-sm">Agenda cheia para este dia.</span>
                </div>`;
            lucide.createIcons();
        }
    } catch (error) {
        // --- ESTADO DE ERRO ---
        if(calendarBtn) calendarBtn.style.display = 'flex'; // Mostra botão para permitir trocar dia
...
// --- CALENDÁRIO COM LOADING NO GRID ---
...
async function openCalendar() {
    ...
    // Mostra loading dentro do calendário enquanto busca dias ocupados
    grid.innerHTML = `
...
function renderCalendarGrid() {
    ...
    for(let i=1; i <= lastDay.getDate(); i++) {
        ...
        if(currentDate.getTime() === today.getTime()) classes += " today border border-gold text-gold";
...
function selectDate(day) {
    ...
    booking.time = null;
    updateDateDisplay();
    // Ao selecionar data, recomeça o processo de loading da tela principal
...
function renderTimeSlots(slots) {
    const container = document.getElementById('time-slots-container');
    let html = '<div class="grid grid-cols-4 gap-3 animate-fadeIn">'; // Animação ao aparecer
...
function selectBarber(id) {
    booking.barber = "Jefferson Almeida";
    setTimeout(() => nextStep(), 300);
    
    // Dispara o carregamento dos horários (que vai ocultar o calendário temporariamente)
    fetchTimeSlots(booking.date);
}
